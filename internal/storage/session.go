...
func (s *SessionStorage) GetSession(ctx context.Context, userID string) (models.Session, error) {
	var session models.Session

	if err := s.coll.FindOne(ctx, bson.D{{"user_id", userID}}).Decode(&session); err != nil {
		return models.Session{}, fmt.Errorf("failed while getting user session: %w", err)
	}
